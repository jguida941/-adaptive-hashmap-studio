...
python hashmap_cli.py config-edit --infile config.toml      # edit existing config (presets supported)
...
```bash
python hashmap_cli.py ab-compare --csv data/workloads/demo.csv \
  --baseline-config config.toml --candidate-config config.experiment.toml \
  --out-dir results/ab/demo
python hashmap_cli.py serve --source results/ab/demo/artifacts/baseline/metrics/metrics.ndjson \
  --compare results/ab/demo/comparison.json
```

The comparison command writes `comparison.json` and `comparison.md` under `--out-dir`, plus per-run metrics archives in `artifacts/`. Point `serve --compare` at the JSON file to surface throughput/latency deltas in the dashboard.
...
- `python hashmap_cli.py --mode adaptive run-csv --csv data/workloads/w_uniform.csv --metrics-port 9090` – dashboard + Prometheus endpoint (Audit Step 3).
...
- `python hashmap_cli.py serve --port 9090 --source runs/metrics.ndjson --follow` – sticky dashboard server fed by NDJSON metrics (now with `/healthz`, `/readyz`, gzip, and optional auth).
...
- Dashboard: `http://localhost:9090/` (inline charts—no CDN dependency, works offline)
...
- Health probes: `http://localhost:9090/healthz`, `http://localhost:9090/readyz`
...
- Robin Hood migrations now require power-of-two capacities. When the live map’s size is not already a power of two we round up to the next one and emit a clear warning (e.g. migrating 6 slots → capacity 8). Workload generators and configs should continue to emit power-of-two capacities for predictable probe behaviour.
