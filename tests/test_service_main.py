...
def test_service_main_invokes_uvicorn_with_expected_configuration(tmp_path: Path) -> None:
    ...
    with (
        mock.patch.object(service_main, "JobManager", job_manager_cls),
        mock.patch.object(service_main, "create_app", create_app_mock),
        mock.patch.object(
            service_main.importlib, "import_module", return_value=uvicorn_module
