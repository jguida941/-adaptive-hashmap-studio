...
def handler_fixture(monkeypatch: pytest.MonkeyPatch) -> Dict[str, Any]:
    ...
    server, stop = start_metrics_server(metrics, port=0)
    stop()
    return captured
...
def handler_with_state_fixture(monkeypatch: pytest.MonkeyPatch) -> Dict[str, Any]:
    ...
    server, stop = start_metrics_server(metrics, port=0, comparison=comparison_payload)
    stop()
...
def test_handler_seeds_token_and_comparison(handler_with_state: Dict[str, Any]) -> None:
    handler_cls: type = handler_with_state["handler"]
    assert handler_cls.api_token == "unit-secret"
...
def test_common_headers_use_canonical_names(handler_info: Dict[str, Any]) -> None:
    handler_cls: type = handler_info["handler"]
...
def test_unauthorized_payload_is_structured(handler_info: Dict[str, Any]) -> None:
    ...
    class FakeHandler:
        ...
        def _limit(self, parsed: Any, default: int, *, clamp: Optional[int] = None) -> int:
            return handler_cls._limit(self, parsed, default, clamp=clamp)
    ...
    class FakeHandler:
        ...
        def _set_common_headers(
            ...
            self.header_args.append(
                {"content_type": content_type, "length": length, "gzip": gzip_enabled}
            )
...
def test_events_payload_schema(handler_with_state: Dict[str, Any]) -> None:
    handler_cls: type = handler_with_state["handler"]
    metrics: Metrics = handler_with_state["metrics"]
    metrics.events_history = [{"idx": 1}, {"idx": 2}]
    captured: Dict[str, Any] = {}
...
def test_events_endpoint_handles_missing_history(handler_info: Dict[str, Any]) -> None:
    handler_cls: type = handler_info["handler"]
    metrics: Metrics = handler_info["metrics"]
    if hasattr(metrics, "events_history"):
        delattr(metrics, "events_history")
    captured: Dict[str, Any] = {}
    ...
    metrics.events_history = None  # type: ignore[assignment]
    captured.clear()
    handler_cls._serve_events(FakeHandler(), urlparse("/api/events?limit=5"))
    assert captured["payload"]["events"] == []

    metrics.events_history = deque([{"idx": 10}, {"idx": 11}])
    captured.clear()
    handler_cls._serve_events(FakeHandler(), urlparse("/api/events?limit=1"))
    assert captured["payload"]["events"] == [{"idx": 11}]

    metrics.events_history = object()  # type: ignore[assignment]
    captured.clear()
...
def test_history_rows_prefers_buffer_and_falls_back(handler_info: Dict[str, Any]) -> None:
    ...
    metrics.history_buffer.extend([{"idx": 0}, {"idx": 1}, {"idx": 2}])
    rows = handler_cls._history_rows(SimpleNamespace(), 2)
    assert rows == [{"idx": 1}, {"idx": 2}]

    metrics.history_buffer = None
    metrics.latest_tick = {"idx": 99}
...
def test_history_rows_handles_missing_attributes(handler_info: Dict[str, Any]) -> None:
    handler_cls: type = handler_info["handler"]
    metrics: Metrics = handler_info["metrics"]
    if hasattr(metrics, "history_buffer"):
        delattr(metrics, "history_buffer")
    if hasattr(metrics, "latest_tick"):
        delattr(metrics, "latest_tick")
    assert handler_cls._history_rows(SimpleNamespace(), 3) == []
    metrics.latest_tick = {"idx": 77}
    assert handler_cls._history_rows(SimpleNamespace(), 1) == [{"idx": 77}]
    metrics.history_buffer = None
    metrics.latest_tick = None
...
def test_serve_metrics_prometheus_encodes_output(handler_info: Dict[str, Any]) -> None:
    handler_cls: type = handler_info["handler"]
    metrics: Metrics = handler_info["metrics"]
    metrics.render = lambda: "# HELP test 1"  # type: ignore[assignment]
    captured: Dict[str, Any] = {}
...
def test_serve_metrics_summary_includes_history(handler_info: Dict[str, Any]) -> None:
    handler_cls: type = handler_info["handler"]
    metrics: Metrics = handler_info["metrics"]
    metrics.history_buffer = deque([{"idx": 1}, {"idx": 2}, {"idx": 3}])
    original_builder = metrics.build_summary_payload
    metrics.build_summary_payload = lambda: {"schema": SUMMARY_SCHEMA, "items": []}  # type: ignore[assignment]
    captured: Dict[str, Any] = {}

    def invoke(parsed: Optional[Any]) -> Dict[str, Any]:
        def _write_json(self: Any, payload: Any, status: int = 200) -> None:
            captured["payload"] = payload
            captured["status"] = status

        def _limit(self: Any, parsed_obj: Any, default: int, *, clamp: Optional[int] = None) -> int:
            return handler_cls._limit(self, parsed_obj, default, clamp=clamp)
        ...
        handler_cls._serve_metrics_summary(FakeHandler(), parsed)
        return captured
    ...
    metrics.build_summary_payload = original_builder
    metrics.history_buffer = None
...
def test_serve_metrics_summary_defaults_to_single_item(handler_info: Dict[str, Any]) -> None:
    handler_cls: type = handler_info["handler"]
    metrics: Metrics = handler_info["metrics"]
    metrics.history_buffer = deque([{"idx": 1}, {"idx": 2}])
    original_builder = metrics.build_summary_payload
    metrics.build_summary_payload = lambda: {"schema": SUMMARY_SCHEMA, "items": []}  # type: ignore[assignment]
    captured: Dict[str, Any] = {}
    ...
    handler = type(
        ...
        (),
        {
            "command": "GET",
            "_write_json": _write_json,
            "_limit": lambda self, parsed_obj, default, *, clamp=None: handler_cls._limit(
                self, parsed_obj, default, clamp=clamp
            ),
